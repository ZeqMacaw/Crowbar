...
#Region "Creation and Destruction"
	...
	Public Sub New(ByVal outputFileStream As StreamWriter, ByVal mdlFileData As SourceMdlFileData10)
...
#Region "Methods"
	...
	Public Sub WriteTrianglesSection(ByVal aBodyModel As SourceMdlModel10)
		Dim line As String = ""
		...
		Try
			If aBodyModel.theMeshes IsNot Nothing Then
				For meshIndex As Integer = 0 To aBodyModel.theMeshes.Count - 1
					...
					If aMesh.theStripsAndFans IsNot Nothing Then
						For stripsAndFansIndex As Integer = 0 To aMesh.theStripsAndFans.Count - 1
							Dim aStripOrFan As SourceMeshTriangleStripOrFan10
							aStripOrFan = aMesh.theStripsAndFans(stripsAndFansIndex)

							If aStripOrFan.theFacesAreStoredAsTriangleStrips Then
								For vertexInfoIndex As Integer = 0 To aStripOrFan.theVertexInfos.Count - 3
									materialLine = materialName

									If vertexInfoIndex Mod 2 = 0 Then
										' even
										vertex1Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex), aTexture)
										vertex2Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex + 2), aTexture)
										vertex3Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex + 1), aTexture)
									Else
										' odd
										vertex1Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex), aTexture)
										vertex2Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex + 1), aTexture)
										vertex3Line = Me.GetVertexLine(aBodyModel, aStripOrFan.theVertexInfos(vertexInfoIndex + 2), aTexture)
									End If
...
#Region "Private Methods"
	...
	Private Function GetVertexLine(ByVal aBodyModel As SourceMdlModel10, ByVal aVertexInfo As SourceMdlVertexInfo10, ByVal aTexture As SourceMdlTexture10) As String
		Dim line As String
		Dim boneIndex As Integer
		Dim vecin As New SourceVector
		Dim position As SourceVector
		Dim normal As SourceVector
		...
		Try
			...
			Dim boneTransform As SourceBoneTransform10
			...
			MathModule.VectorCopy(aBodyModel.theVertexes(aVertexInfo.vertexIndex), vecin)
			...
			MathModule.VectorCopy(aBodyModel.theNormals(aVertexInfo.normalIndex), vecin)
			...
			MathModule.VectorNormalize(normal)
